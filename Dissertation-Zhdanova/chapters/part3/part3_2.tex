...
Будем рассматривать скрытую полумарковскую модель Фергюсона $\lambda = \{S,A,\pi,V,B,D,F\}$, параметры которой определяются в соответствии с (\ref{chapt2_Ferguson_model:def}), и некоторую последовательность символов $O_{1:T}$ из алфавита $V$ -- $O_{1:T}$.
\begin{theorem}
...
\end{array}\right.
\end{equation}
Воспользуемся тем, что в модели Фергюсона вероятность наблюдения символа зависит только от состояния, но не от его длительности. Тогда справедливо
