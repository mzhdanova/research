...
\begin{theorem}\label{chapt3_theorem_Ferguson}
...
\begin{array}{ll}
1 ,&t\leq 0,\\
\sum\limits_{j\in \mathcal{S}}\sum\limits_{d \in \mathcal{D}}\sum\limits_{d_1=1}^{d}\widetilde{P}[O_{1:t-d}]\overline{\alpha}_{t-d_1+1}(j,d)\prod\limits_{\tau=t-d_1+1}^{t-d_1+d} b_{j,d}(O_\tau),& t\in [1,T],\end{array}\right.
\end{equation}
\begin{equation}\nonumber
\overline{\alpha}_{t}(i,d)=\left\{
\begin{array}{ll}
\pi_i\rho_i(d), &t\leq 0,\\
\sum\limits_{i^\prime\in \mathcal{S}}\sum\limits_{d^\prime \in \mathcal{D}}\overline{\alpha}_{t-d}(i^\prime,d^\prime)\prod\limits_{\tau=t-d-d^\prime+1}^{t-d} a_{i^\prime i} p_i(d)b_{i^\prime,d^\prime}(O_\tau)\frac{P[O_{1:t-d-d^\prime}]}{P[O_{1:t-d}]}
, & t\in [1,T].
\end{array}\right.
\end{equation}
