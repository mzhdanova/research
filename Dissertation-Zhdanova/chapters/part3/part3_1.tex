...
В разделе \ref{chapt3_GHSMM_Theorems} будет рассмотрена задача оценивания для общей скрытой полумарковской модели при более общих, чем в \ref{chapt1_GHSMM_IP_Yu}, предположениях относительно начала и конца наблюдаемой части последовательности состояний. Будет предложено два варианта решения этой задачи.

В разделе \ref{chapt3_GHSMM_Auxilary} будут изложены вспомогательные леммы, необходимые для решения обратной задачи для подкласса общих скрытых полумарковских моделей -- моделей с независимыми наблюдениями. Под независимостью наблюдений понимается взаимная независимость символов, порожденных одним состоянием. Частными случаями общих скрытых полумарковских моделей с независимыми наблюдениями являются скрытая полумарковская модель Фергюсона и скрытая полумарковская модель $QP$-модель. Приведенные в этом разделе леммы будут использоваться в разделах \ref{chapt3_Ferguson} и \ref{chapt3_HSMQPM} для решения задач оценивания для упомянутых моделей.
...
В разделе \ref{chapt1_GHSMM_IP_Yu} рассматривался подход Ю к решению задачи оценивания для общей скрытой полумарковской модели при предположении, что состояние $r_1$ началось в момент времени $t=1$ или до него и что последнее состояние $r_q$ закончилось строго в момент времени $T$. Будем вслед за Ю говорить об этом предположении как о смешанном (mixed assumption). В настоящем разделе мы предложим решение задачи оценивания для общей скрытой полумарковской модели при общем предположении (general assumption), а именно, что:
...
Отличие от условий, выделенных Ю в \cite{Yu} (см. начало раздела \ref{chapt1_GHSMM_IP_Yu}), состоит в том, что здесь рассматриваются менее обременительные ограничения на окончание последнего состояния. Об этом предположении будем далее говорить как об общем (general assumption).
...
%Д-замечание. Величины $alpha_T(i,d)$ и $\overline{alpha}_T(i,d)$ определяются у Ю только при $t>0$. При $t<=0$ для них нет определения. Поэтому, наверное, мы можем определить их как хотим.
...
\end{theorem}
\begin{proof}
Рассмотрим $\widetilde{P}[O_{1:T}]$. Построим разбиение пространства элементарных событий $\mathfrak{E}=\{E_{j,d,d_1}\}_{j\in \mathcal{S},d\in \mathcal{D}, d_1\in[1,d]}$, $P[E_{j,d,d_1}]>0, j\in S, d\in D, d_1\in[1,d]$, представляющее собой полную группу несовместимых событий. Событие $E_{j,d,d_1}$ состоит в том, что в момент времени $T$ модель находится в состоянии $j$ длительностью $d$, причём на момент времени $T$ прошло $d_1(\leq d)$ моментов времени. Тогда по формуле полной вероятности можем записать:
...
= \sum\limits_{i\in S}\sum\limits_{d^\prime \in D} P[S_{[T-d_1-d^\prime+1:T-d_1]}=i]\\
& \cdot P[S_{[T-d_1+1, T-d_1+d]}=j, O_{1:T}| S_{[T-d_1-d^\prime+1:T-d_1]}=i]\\
&= \sum\limits_{i\in S}\sum\limits_{d^\prime \in D}P[S_{[T-d_1+1, T-d_1+d]}=j, O_{1:T}, S_{[T-d_1-d^\prime+1:T-d_1]}=i]\\
&= \sum\limits_{i\in S}\sum\limits_{d^\prime \in D}P[S_{[T-d_1+1, T-d_1+d]}=j, O_{1:T-d_1}, O_{T-d_1+1:T}, S_{[T-d_1-d^\prime+1:T-d_1]}=i] \\
&= \sum\limits_{i\in S}\sum\limits_{d^\prime \in D} P[O_{1:T-d_1}, S_{[T-d_1-d^\prime+1:T-d_1]}=i] \\
...
Учитывая, что общая скрытая полумарковская модель предполагает условную независимость событий $S_{[T-d_1+1, T-d_1+d]}=j$ и $O_{1:T-d_1}$ при условии события $S_{[T-d_1-d^\prime+1:T-d_1]}=i]$, перепишем (\ref{eq_cond_1}):
\begin{equation}\nonumber
\begin{split}&P[S_{[T-d_1+1, T-d_1+d]}=j, O_{1:T-d_1}|S_{[T-d_1-d^\prime+1:T-d_1]}=i] =\\& P[S_{[T-d_1+1, T-d_1+d]}=j |S_{[T-d_1-d^\prime+1:T-d_1]}=i]P[O_{1:T-d_1}|S_{[T-d_1-d^\prime+1:T-d_1]}=i].
\end{split}
\end{equation}
Тогда перепишем:
\begin{equation}\nonumber
...
\end{split}
\end{equation}
Поскольку общая скрытая полумарковская модель предполагает условную независимость событий $O_{T-d_1+1:T}$ и $S_{[T-d_1+1, T-d_1+d]}=j$ при условии события $O_{1:T-d_1}\cdot S_{[T-d_1-d^\prime+1:T-d_1]}=i]$, по (\ref{eq_cond_2}) запишем:
\begin{equation}\nonumber
\begin{split}
&P[O_{T-d_1+1:T}, S_{[T-d_1+1, T-d_1+d]}=j|O_{1:T-d_1}, S_{[T-d_1-d^\prime+1:T-d_1]}=i]=\\
...
\end{split}
\end{equation}
Перепишем $P[S_{[T-d_1+1, T-d_1+d]}=j|O_{1:T-d_1}, S_{[T-d_1-d^\prime+1:T-d_1]}=i]$ в виде:
\begin{equation}\nonumber
...
\begin{equation}\nonumber
\begin{split}
& \widetilde{P}[O_{1:T}]=\sum\limits_{j\in \mathcal{S}}\sum\limits_{d \in \mathcal{D}}\sum\limits_{d_1=1}^{d}\sum\limits_{i\in S}\sum\limits_{d^\prime \in \mathcal{D}}P[O_{1:T-d_1}, S_{[T-d_1-d^\prime+1:T-d_1]}=i]\\
&\cdot P[S_{[T-d_1+1, T-d_1+d]}=j| S_{[T-d_1-d^\prime+1:T-d_1]}=i]P[O_{T-d_1+1:T}|S_{[T-d_1+1, T-d_1+d]}=j].
\end{split}
\end{equation}
Преобразуем полученное выражение, выражая множители через известные величины:
\begin{equation}\nonumber
\begin{split}
&\widetilde{P}[O_{1:T}]=\\&\sum\limits_{j\in \mathcal{S}}\sum\limits_{d \in \mathcal{D}}\sum\limits_{d_1=1}^{d}\sum\limits_{i\in S}\sum\limits_{d^\prime \in D}P[O_{1:T-d_1}, S_{[T-d_1-d^\prime+1:T-d_1]}=i]a_{(i,d^\prime)(j,d)} b_{j,d}(O_{T-d_1+1}^{T+d-d_1}).
\end{split}
\end{equation}
Заметим так же, что $P[O_{1:T-d_1}, S_{[T-d_1-d^\prime+1:T-d_1]}=i]$ представляет собой прямую переменную  $\alpha_{T-d_1}(i,d^\prime)$ (\cite{Yu}, с. 218), определяемую как совместную вероятность двух событий -- наблюдать последовательность $O_{1:T-d_1}$ и, начиная с момента $T-d_1+d^\prime+1$ и заканчивая моментом $T-d_1$, наблюдать состояние $i$. Тогда:
\begin{equation}\nonumber
\begin{split}
...
\end{split}
\end{equation}
Для вычисления $\alpha_{t}(j,d)$ Ю предлагает использовать формулу (4) c.218. Согласно ей запишем:
\begin{equation}
\nonumber
\alpha_{t}(j,d)=\left\{
\begin{array}{ll}
...
Таким образом,
\begin{equation}
\label{chapt3_th1_proof}
\widetilde{P}[O_{1:T}]=\sum\limits_{j\in \mathcal{S}}\sum\limits_{d \in \mathcal{D}}\sum\limits_{d_1=1}^{d}\alpha_{T-d_1+d}(j,d).
\end{equation}
Преобразуем формулу (\ref{chapt3_th1_proof}), используя понятие апостериорной вероятности $\overline{\alpha}_{t}(j,d)$. Напомним, что $\overline{\alpha}_{t}(j,d)$ определяется следующим образом:
$$\overline{\alpha}_{t}(j,d) = P[S_{[t-d+1:t]}=j|O_{1:t-d}]\;\;\; \forall t>0$$
и имеет место следующее соотношение:
\begin{equation}
\label{chapt3_th1_proof_post_alpha}
{\alpha}_{t}(j,d) = \widetilde{P}[O_{1:t-d}]\overline{\alpha}_{t}(j,d)b_{j,d}(O_{t-d+1}^t).
\end{equation}
 Подставляя (\ref{chapt3_th1_proof_post_alpha}) в (\ref{chapt3_th1_proof}), получим
\begin{equation}\nonumber
\widetilde{P}[O_{1:T}]=\sum\limits_{j\in \mathcal{S}}\sum\limits_{d \in \mathcal{D}}\sum\limits_{d_1=1}^{d}\widetilde{P}[O_{1:T-d_1}]\overline{\alpha}_{T-d_1+d}(j,d)b_{j,d}(O_{T-d_1+1}^{T-d_1+d}).
\end{equation}
...
Для инициализации рекурсии необходимо определить также начальные значения. Определим начальные значения при $\forall t\leq 0$ следующим образом:
$$\overline{\alpha}_{t}(i,d)=\pi_{i,d},$$
$$\widetilde{P}[O_{1:t}]=1.$$
\end{proof}

\begin{remark} В \cite{Yu} без обоснования содержится формула для решения задачи оценивания в более узком, чем в \ref{th:chapt3_GHSMM:1}, случае --- случае смешанного предположения. В [кн Ю] предлагается решение задачи оценивания в случае общего предположения, использующее, однако, иное определение прямой переменной и не снабженное детальным обоснованием.\end{remark}

%(НЕ ЗАБЫТЬ: Теорему 1 аккуратно включить в результаты, выносимые на защиту)
...
\begin{theorem}\label{th:chapt3_GHSMM:2} Будем предполагать последовательность $O_{1:T}$ настолько длинной, что вероятность последнего наблюдаемого состояния удовлетворяет предельному распределению вероятностей.
...
\begin{equation}\nonumber
\begin{split}
O_{1:T}= & O_{1:T}\bigcap(\bigcup\limits_{(j,d)\in\mathcal{S}\times \mathcal{D}}E^{j,d})\bigcap(\bigcup\limits_{d_1=1}^{D}E_{d_1})= \\
&O_{1:T}\bigcap(\bigcup\limits_{(j,d)\in\mathcal{S}\times \mathcal{D}}\bigcup\limits_{d_1=1}^{D}E^{j,d}\bigcap E_{d_1})=\\
&\bigcup\limits_{(j,d)\in\mathcal{S}\times \mathcal{D}}\bigcup\limits_{d_1=1}^{D}E^{j,d}\bigcap E_{d_1}\bigcap O_{1:T}.
\end{split}
\end{equation}
Далее для произвольных событий $A_1,..,A_n$ будем использовать обозначение:
$$P[A_1,..,A_n]=P[\bigcap\limits_{i=1}^{n}A_i].$$
Ввиду (\ref{eq:chapt3:theorem2:proof:1}) из последнего соотношения вытекает, что
...
События $O_{1:T-d_{1}}$ и $O_{T-d_{1}+1:T}$ являются условно независимыми относительно $E^{j,d}\cap E_{d_{1}}$. Поэтому в виду (ссылка на свойство условной независимости)
$$P[O_{1:T-d_1}, O_{T-d_1+1:T}|E^{j,d},E_{d_1}]=P[O_{1:T-d_1}|E^{j,d},E_{d_1}]P[O_{T-d_1+1:T}|E^{j,d},E_{d_1}.$$
События $O_{1:T-d_1}$ и $E^{j,d}$ условно независимы относительно $E_{d_1}$, поэтому
...
События $O_{T-d_{1}+1:T}$ и $E_{d_1}$ условно независимы относительно $E^{j,d}$, поэтому
...
Таким образом, подставляя полученные выражения для $P[E^{j,d}, E_{d_1}]$ и  $P[O_{1:T}|E^{j,d}, E_{d_{1}}]$  в формулу (\ref{eq:chapt3:theorem2:proof:2}), получим:
\begin{equation}\nonumber
P[ O_{1:T}] =
...
Рассмотрим два события $A$ и $B$ и пусть $P[A, B]$ -- совместная вероятность этих двух событий. По маргинальной вероятностью события $A$ будем понимать безусловную вероятность $P[A]$ события $A$, т.е., вероятность события $A$, независимого от того, наступает ли какое-то другое событие $B$ или нет. Если о $B$ можно думать как о некоторой случайной величине, принявшей данное значение, маргинальная вероятность $A$ может быть получена суммированием (или более широко интегрированием) совместных вероятностей по всем значениям этой случайной величины
...
\end{enumerate}
\end{lemma}
\begin{proof} Рассмотрим сначала ситуацию (\ref{eq:chapt3_GHSMM_Aux_L1_1}), т.е. $t+d>0$ и $t\leq T$.
...
\end{lemma}
\begin{proof}
Рассмотрим случай, когда $t\leq 0$. Будем предполагать, что первое состояние начинается в или до момента времени $t^\prime=1$.
Рассмотрим определение $\overline{\alpha}_t(i,d)$ из раздела \ref{chapt1_GHSMM_IP_Yu} (см. (\ref{eq: chapt1_InverseProbles_Yu_Posterior_Alpha})) и расширим его на случай $t\leq 0$:
$$\forall t\leq 0,\forall (i,d) \in \mathcal{S}\times \mathcal{D}:\;\;\; \overline{\alpha}_t(i,d)= P[S_{[t-d+1:t]} = i| O_{1:t}].$$
Используя определение условной вероятности, правую часть можно преобразовать к виду
$$P[S_{[t-d+1:t]} = i| O_{1:t}]=\frac{P[S_{[t-d+1:t]}=i, O_{1:t}]}{P[O_{1:t}]}.$$
Рассмотрим
$$P[S_{[t-d+1:t]}=i, O_{1:t}]= P[S_{[t-d+1:t]}=i]=P[O_{1:t}|S_{[t-d+1:t]}=i].$$
Так как мы не знаем, какие символы наблюдались до момента времени $t< 1$, $$P[O_{1:t}|S_{[t-d+1:t]}=i]=1$$ при $t<1$. Следовательно,
$$P[S_{[t-d+1:t]}=i, O_{1:t}] =P[S_{[t-d+1:t]}=i] = \pi_{i,d}=\pi_i p_i(d)$$
и
$$\overline{\alpha}_t(i,d)= P[S_{[t-d+1:t]} = i| O_{1:t}] = \frac{\pi_i p_i(d)}{P[O_{1:t}]}.$$
Рассмотрим  полную группу несовместимых событий $\mathfrak{E} = \{E_{i,d}\}_{i\in \mathcal{S}, d\in \mathcal{D}}$, где событие $E_{i,d}$  состоит в том, что в момент времени $t$ закончилось состояние $i$ длительностью $d$. По формуле полной вероятности можем записать
$$P[O_{1:t}]=\sum\limits_{i\in \mathcal{S}}\sum\limits_{d\in \mathcal{D}}P[S_{[t-d+1:t]}=i, O_{1:t}].$$
Тогда, при $t\leq 0$ получим:
$$P[O_{1:t}]=\sum\limits_{i\in \mathcal{S}}\sum\limits_{d\in \mathcal{D}}\pi_{i,d}=1.$$
Пользуясь формулой (\ref{eq:chapt1_GHSMM_Indep:inclus3}),
можем записать
$$\forall t\leq 0, \forall (i,d) \in \mathcal{S}\times \mathcal{D}: \overline{\alpha}_t(i,d)=\pi_{i} p_{i} (d).$$
Для доказательства утверждения при $0<t\leq T$ рассмотрим формулы (\ref{eq: chapt1_InverseProbles_Yu_Probability}, \ref{eq: chapt1_InverseProbles_Yu_Posterior_Alpha_Recursion}):
 \begin{equation}
\nonumber
...
\end{equation}
 \begin{equation}
\nonumber
\overline{\alpha}_t(i,d) = \sum\limits_{i^\prime\in \mathcal{S}}\sum\limits_{d^\prime \in \mathcal{D}}\overline{\alpha}_{t-d}(i^\prime,d^\prime)\frac{b_{i^\prime,d^\prime}(O_{t-d-d^\prime+1}^{t-d})P[O_{1:t-d-d^\prime}]}{P[O_{1:t-d}]}a_{(i^\prime,d^\prime)(i,d)}.
\end{equation}
Напомним, что для скрытой полумарковской модели с относительно независимыми длительностями и наблюдениями справедливо соотношение (\ref{eq:chapt1_GHSMM_Indep:inclus1}, \ref{eq:chapt1_GHSMM_Indep:inclus3}):
\begin{equation} a_{(i,d)(j,d^\prime)}= a_{ij}\cdot p_j(d^\prime),\end{equation}
где $a_{ij}=P[S_{[t=j}|S_{t-1]=i}]$.

Подставляя (\ref{eq:chapt1_GHSMM_Indep:inclus1}) в (\ref{eq: chapt1_InverseProbles_Yu_Posterior_Alpha_Recursion}), получим:
 \begin{equation}
\nonumber
\overline{\alpha}_t(i,d) = \sum\limits_{i^\prime\in \mathcal{S}}\sum\limits_{d^\prime \in \mathcal{D}}\overline{\alpha}_{t-d}(i^\prime,d^\prime)\frac{b_{i^\prime,d^\prime}(O_{t-d-d^\prime+1}^{t-d})P[O_{1:t-d-d^\prime}]}{P[O_{1:t-d}]}a_{ii^\prime}\cdot p_i^\prime(d^\prime).
\end{equation}
Используя лемму \ref{lemma:chapt3_GHSMM_Aux_L1}, преобразуем:
 \begin{equation}
\nonumber
\overline{\alpha}_t(i,d) = \sum\limits_{i^\prime\in \mathcal{S}}\sum\limits_{d^\prime \in \mathcal{D}}\overline{\alpha}_{t-d}(i^\prime,d^\prime)\prod\limits_{\tau=t-d-d^\prime+1}^{t-d}b_{i^\prime,d^\prime}(O_\tau)\frac{P[O_{1:t-d-d^\prime}]}{P[O_{1:t-d}]}a_{ii^\prime}\cdot p_i^\prime(d^\prime).
\end{equation}
С учетом леммы \ref{lemma:chapt3_GHSMM_Aux_L1} выражение(\ref{eq: chapt1_InverseProbles_Yu_Probability}) преобразуется к виду:
  \begin{equation}
\nonumber
P[O_{1:t}]=\sum\limits_{i\in \mathcal{S}}\sum\limits_{d \in \mathcal{D}} P[O_{1:t-d}]\overline{\alpha}_t(i,d)\prod\limits_{\tau=t-d+1}^t b_{i,d}(O_\tau)
\end{equation}
%Для инициализации $\overline{\alpha}_t(i,d)$ используем (\ref{eq: chapt1_InverseProbles_Yu_Posterior_Alpha_Init0}, \ref{eq: chapt1_InverseProbles_Yu_Posterior_Alpha_InitNegative}) с учетом (\ref{eq:chapt1_GHSMM_Indep:inclus1})
%\begin{equation}
%\overline{\alpha}_0(j,d) = \pi_{j}\cdot p_j(d),
%\end{equation}
%\begin{equation}
%\overline{\alpha}_t(j,d) = 0, \;\;\; t<0.
%\end{equation}
Объединив случаи $t\leq 0$ и $0<t\leq T$ в одном выражении, можем записать:
\begin{equation}\nonumber
P[O_{1:t}]=\left\{
\begin{array}{ll}
1 ,&t\leq 0,\\
