...
\begin{equation}\nonumber a_{(i^\prime,d^\prime)(i,d)}= a_{{i^\prime} i}\cdot p_{i^\prime}(d^\prime),\end{equation}
\begin{equation}
\nonumber
...
\begin{equation}
\widetilde{P}[O_{1:t}]=\left\{\begin{array}{ll}
1, &t\leq 0,\\\sum\limits_{j\in \mathcal{S}}\sum\limits_{d \in \mathcal{D}}\sum\limits_{d_1=1}^{d}\widetilde{P}[O_{1:t-d_1}]\overline{\alpha}_{t-d_1+d}(j,d)\prod\limits_{\tau=t-d_1+1}^{t-d_1+d} b_{j,d}(O_\tau),& t\in [1,T],\end{array}\right.
\end{equation}
Теорема доказана.
\end{proof} 