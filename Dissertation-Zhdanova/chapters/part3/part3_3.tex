\section{Решение задачи оценивания для скрытой полумарковской $QP$-модели} \label{chapt3_HSMQPM}
 В этом разделе сформулируем и докажем теорему о решении задачи оценивания для скрытой полумарковской $QP$-модели (см. раздел \ref{chapt2_HSMQPN_HSMQPN}).

Прежде чем перейти к решению задачи оценивания, сформулируем необходимое нам в дальнейшем вспомогательное утверждение.

Зафиксируем скрытую полумарковскую $QP$-модель $\lambda=\{\mathcal{S};\mathcal{D}; A; \Pi; p(d); \rho; \mu ;M ;{\mathbb{F}}_q;B \}$. Пусть $o$ ---  некоторый символ из алфавита $\mathbb{F}_q$. Обозначим $b_{i, d}^{\tau}(o)$ вероятность наблюдать символ $o$ в состоянии $i$ на $\tau$-ой позиции внутри отрезка длины $d$. Обозначим $I_{\mathbb{F}^{\ast}_q}$ --- индикатор множества $\mathbb{F}^{\ast}_q$. Под $\varphi_i^d$ будем понимать функцию, полученную масштабным растяжением эталонной плотности $\rho_i$  на отрезок длины $d$ согласно (\ref{eq_varphi}) (подробнее см. в \cite{DeMoRGUPS}).

\begin{lemma}\label{chapt3_lemma3}
Вероятность $b_{i, d}^{\tau}(o)$ может быть вычислена по формуле:

\begin{equation}
\label{eq:chap3:l3:1}
b_{i, d}^{\tau}(o) =I_{\mathbb{F}^{\ast}_q}(o)\varphi_i^d(\tau)\mu_idb_i(o)+(1-I_{\mathbb{F}^{\ast}_q}(o))(1-\varphi_i^d(\tau)\mu_id).
\end{equation}
\end{lemma}
\begin{proof}
В соответствии с (\ref{eq_error_prob}) в каждый момент времени $\tau$ внутри длительности $d$ в состоянии $i$ мы можем наблюдать ненулевой символ (то есть символ из $\mathbb{F}_q^\ast$) с вероятностью $e_{i,d}^\tau= \varphi_i^d(\tau)\mu_i d$ и нулевой символ (не принадлежащий $\mathbb{F}_q^\ast$) с вероятностью $1-e_{i,d}^\tau$. В случае, когда символ ненулевой, его значение определяется соответствующей текущему состоянию $i$ строкой матрицы $B$. Тогда, для краткости используя понятие индикатора множества, можно записать (\ref{eq_b}).
\end{proof}

Теперь сформулируем теорему о решении задачи оценивания для скрытой полумарковской $QP$-модели.
\begin{theorem}
Вероятность генерации последовательности $O_{1:T}$ скрытой полумарковской $QP$-моделью $\lambda$ при общем предположении может быть вычислена по следующей формуле:
\begin{equation}\nonumber
...
\end{array}\right.
\end{equation}
В соответствии с леммой \ref{chapt3_lemma3} вероятность $b_{i^\prime,d^\prime}(O_\tau)=b_{i,d}^{\tau}(O_\tau)$ может быть вычислена в соответствии с формулой (\ref{eq:chap3:l3:1}).
\end{proof}
